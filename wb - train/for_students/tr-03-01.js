...
function draw(canvas, param) {
  ...
  for (let i = 0; i < thePoints.length; i++) {
    ...
    let previous = thePoints[(i - 1 + thePoints.length) % thePoints.length];
    let current = thePoints[i];
    let next = thePoints[(i + 1) % thePoints.length];
    let next2 = thePoints[(i + 2) % thePoints.length];

   if (document.getElementById("check-b-spline").checked) {
      context.moveTo(current[0], current[1]);
      let xc = (Math.pow(1 - tension, 2) * previous[0]) + ((1 - tension) * 2 * tension * current[0]) + (Math.pow(tension, 2) * next[0]),
          yc = (Math.pow(1 - tension, 2) * previous[1]) + ((1 - tension) * 2 * tension * current[1]) + (Math.pow(tension, 2) * next[1]);

      context.quadraticCurveTo(xc, yc, next[0], next[1]);
      context.stroke();
    } else {
      let cp1 = current[0] + (((next[0] - previous[0]) * tension) / 3),
          cp2 = current[1] + (((next[1] - previous[1]) * tension) / 3),

          cp3 = next[0] - (((next2[0] - current[0]) * tension) / 3),
          cp4 = next[1] - (((next2[1] - current[1]) * tension) / 3);

      context.moveTo(current[0], current[1]);
      context.bezierCurveTo(cp1, cp2, cp3, cp4, next[0], next[1]);
      context.stroke();
    }   
  } 
}
...
let tensionSlider = document.getElementById("tensionSlider");
let speedSlider;
