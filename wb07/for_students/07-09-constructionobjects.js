...
export class GrDumpTruck extends GrObject {
  ...
  constructor( params = {} ) {
    ...
    let wheelGeom = new T.BoxGeometry(3, 3, 8.25);
      let wheelMat = new T.MeshLambertMaterial({ color: 0x000000 });
      ...
      let bedGeom = new T.BoxGeometry(8, 3.75, 7.5);
      let bedMat = new T.MeshStandardMaterial({
        color: "yellow",
        metalness: 0.5,
        roughness: 0.7
      });
      ...
      body.position.y = 3;
      let bed = new T.Mesh(bedGeom, bedMat);
      bed.position.y = 6.75;
      bed.position.x = 3.5;

      let group2 = new T.Group();
      group2.add(bed);
      group2.position.set(0, 0, 0);

      dumpTruck.add(body, group2, frontWheel, backWheel);
      dumpTruck.scale.set(0.2, 0.15, 0.15);

  super("GrDumpTruck", dumpTruck, [
    ["x", -5, 5, 2],
    ["z", -5, 5, 2],
    ["theta", -180, 180, 0],
    ["tilt", 0, 90, 0]
  ...
  update(paramValues) {
    ...
    this.whole_ob.position.z = paramValues[1];
    this.whole_ob.rotation.y = degreesToRadians(paramValues[2]);
    this.bed.rotation.z = degreesToRadians(-paramValues[3]);
  }
}